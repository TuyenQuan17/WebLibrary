...
namespace GaziLibrary.Controllers
{
    public class AuthController : Controller
    {
        ...
        public async Task<IActionResult> Login(User user)
        {
            ...
            if (!result.Success)
            {
                _notyf.Error("Mật khẩu hoặc tên người dùng không chính xác", 3);
                var model = new UserModel
                {
                    User = new User()
                };
            ...
            _notyf.Success("Chào mừng " + user.UserName, 3);

            if (result2.Data.Position.Name == "USER")
            {
                return RedirectToAction("MainScreen", "User");
            }
        ...
        {
            user.Status = true;
            user.PositionId = _positionService.GetByName("USER").Data.Id;
            ...
            if(!result.Success)
            {
                _notyf.Error("Đã xảy ra lỗi khi thực hiện giao dịch thành viên.");
                var model = new UserModel
                {
                    User = new User()
                };
            ...
            _notyf.Success("Quá trình thành viên đã hoàn tất thành công.");

...
namespace GaziLibrary.Business.Concrete
{
    public class BookManager : IBookService
    {
        ...
        {
            string searchUpper = search.ToUpper(); // Convert search to uppercase
            var result = _bookDal.GetAllByFK(b => b.Status == true &&
                (b.Name.ToUpper().Contains(searchUpper) ||
                b.Author.FirstName.ToUpper().Contains(searchUpper) ||
                b.Author.LastName.ToUpper().Contains(searchUpper) ||
                b.Type.Name.ToUpper().Contains(searchUpper)));
